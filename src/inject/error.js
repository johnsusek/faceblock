...
window.logException = function(ex) {
  ...
  console.info(`[nocontrol] Caught exception ${uuid}. Please include this identifier if reporting a bug.`, exception);
};

window.logError = function(error) {
  const uuid = generateUuid();
  window.logger({ error }, uuid);
  console.info(`[nocontrol] Caught error ${uuid}. Please include this identifier if reporting a bug.`, error);
};

window.logger = function(payload, uuid = generateUuid()) {
  window
    .postJSON('https://log.declaredintent.com/entries', {
      namespace: 'com.declaredintent.nocontrol',
